...
from bangul_client import giveserver
...
#모터 초기화
now_angle = motor.camera_motor_control(90, 0)
...
while True:
    ...
    for i in range(len(scores)) :
        if ((scores[i] > 0.5) and (scores[i] <= 1.0) and labels[int(classes[i])] in detect_list):
            ymin = int(max(1,(boxes[i][0] * imH)))
            xmin = int(max(1,(boxes[i][1] * imW)))
            ymax = int(min(imH,(boxes[i][2] * imH)))
            xmax = int(min(imW,(boxes[i][3] * imW)))
            gridbox.append([ymin,xmin,ymax,xmax,labels[int(classes[i])]) #그리드 박스에 좌측 상단, 우츤 하단 점을 저장한다
    ...
    for ymin,xmin,ymax,xmax,label in gridbox :
        ...
        values[label][cnt[label]]['point'] = real_coordinate([xmin,FY-ymax] , [xmax, FY-ymax]) #하단 중앙점의 현실 좌표계를 반환합니다
        values[label][cnt[label]]['gridbox'] = [ymin,xmin,ymax,xmax]
        #ROI RGB
        roi_px = (xmin+xmax) * roi_dict[label]['x']
        roi_py = (ymin+ymax) * roi_dict[label]['y']
        roi_color = []
        for i in range(roi_px-ROI_LEN_X , roi_px+ROI_LEN_X+1 ) :
            for j in range(roi_py-ROI_LEN_Y , roi_py+ROI_LEN_Y+1 ) :
                if i>=0 and i<=PX and j>=0 and j<=PY :
                    roi_color.append(frame_resized[i][j])
        values[label][cnt[label]]['color'] =  determine_color(roi_color)

            
    #----- 3. 추적 알고리즘 시행, cv2를 통한 라벨 생성부   ------#
    # 추적 알고리즘 시행
    exists = track.track(exists,values,detect_list)
    ...
    for object_label in detect_list :
        ...
        for i in range(LEN_EXIST) :
            if exist_list[i].detect == True :
                ...
                server_label[object_name] = {'x' : xmin , 'y' : label_ymin-labelSize[1]-10}
